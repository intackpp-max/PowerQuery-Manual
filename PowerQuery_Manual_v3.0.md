...
## 🎯 왜 Power Query를 사용하는가?

### 핵심 이유: 행정 원본 변경 → 농협 템플릿 자동 업데이트
...
[농협 고정 템플릿]
이름, 생년월일, 핸드폰, 주소, 조합원여부, 신청항목, 수량...
(항상 동일한 형식 유지)
```

---

### Power Query를 쓰면 얻는 3가지 핵심 효과

① **시간 절약**: 3시간 → 5분 (36배 빠름)
② **오류 제로**: 수동 입력 없음 → 오타 방지
③ **표준화**: 모든 직원이 동일한 방식으로 작업
